...
describes what users get. Pick a format with `-o`, or let unsdg choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
unsdg <command> -o table   # aligned columns for reading
unsdg <command> -o jsonl   # one JSON object per line, for piping
unsdg <command> -o json    # a single JSON array
unsdg <command> -o csv     # spreadsheet friendly
unsdg <command> -o tsv     # tab-separated
unsdg <command> -o url     # just the URL column
unsdg <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
unsdg <command> --fields id,title,url
```
...
```bash
unsdg <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
unsdg <command>            # a table, because this is a terminal
unsdg <command> | wc -l    # JSONL, because this is a pipe
```
